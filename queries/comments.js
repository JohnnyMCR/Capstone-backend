...
const createComment = async (commentToAdd) => {
    try {
        const newComment = await db.one(`INSERT INTO comments (post_id, user_id, content, date) VALUES ($1, $2, $3, $4)` [commentToAdd.post_id, commentToAdd.user_id, commentToAdd.content, commentToAdd.date])
        return newComment
    } catch (error) {
        return error
    }
